...
void generateSolution(struct Room **roomList)
{
	...
	roomList[0]->roomType = -1;
	roomList[solutionRoomCount - 1]->roomType = 1;
...
void createConnections(struct Room **roomList)
{
	int i, j, totalConnect, randRoom, compareName, connExists;
	for (i = 0; i < 7; i++)
	{
...
/* printCharArray() is a debugging tool. Prints out the contents of an array of
	char arrays.
	Preconditions: array of char arrays has values
	Postconditions: array of char arrays is printed out
	Receives: pointer to array of char arrays; length of the array
	Returns: none */
...
/* printRoom is a debugging tool. Prints out the information for a supplied
	Room.
	Preconditions: Room values have been set
	Postconditions: Room information is printed out
	Receives: pointer to Room.
	Returns: none */
void printRoom(struct Room *room)
{
	int i;
	printf("ROOM NAME: %s\n", room->roomName);
	for (i = 0; i < room->connNum; i++)
	{
		printf("CONNECTION %d: %s\n", i + 1, room->connections[i]->roomName);
	}
	printf("ROOM TYPE: ");
	if (room->roomType == -1)
	{
